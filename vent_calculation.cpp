 ...
 */
double vent_calculation::get_FSSA_vent_area(double ext_vent_area, double pressure_limit , double safety_factor_area ,double rel_humidity , int hd_index)
{
    if(agent_index == 1){
        ...
        if(design_concentration > 4.2 && design_concentration < 6){
            ...
            if(discharge_time > 6 && discharge_time <10){
                bool evaflag = false;
                if(ext_vent_area > 0){
                    double pos_pressure = (467.6 + (2.696*rel_humidity)) * (pow(((ext_vent_area)/(encl_volume*discharge_time)/design_concentration), (-1.0334)));
                ...
                double pos_pressure = (467.6 + (2.696*rel_humidity)) * (pow(((pos_vent_area)/(encl_volume*discharge_time) / design_concentration), (-1.0334)));
        ...
        if(design_concentration > 6.25 && design_concentration < 10.5){
            ...
            if(discharge_time > 6 && discharge_time < 10){
                ...
                double neg_pressure = (7251-77.29*(rel_humidity))*((1-0.3935) * log(((neg_vent_area)/(encl_volume*discharge_time) / design_concentration)));
